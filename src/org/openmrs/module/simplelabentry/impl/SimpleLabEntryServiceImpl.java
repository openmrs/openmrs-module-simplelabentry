...
public class SimpleLabEntryServiceImpl extends BaseOpenmrsService implements
    ...
    /**
     * @see org.openmrs.module.simplelabentry.SimpleLabEntryService#getOrders(OrderType,
     *      Concept, Location, Date, ORDER_STATUS, List<Patient>)
     */
    ...
    public List<Map<String, String>> getLabOrderReportData(Location location,
        ...
        for (Encounter encounter : encounters) { // used to be
            ...
            for (ConceptColumn conceptColumn : getConceptColumns()) {
                ...
                row.put(conceptColumn.getDisplayName() + " FAILURE",
                        LabResultListItem.getReadibleFailureStringFromObs(obs));
            }
            // Add row to dataset
            dataset.add(row);
        }
